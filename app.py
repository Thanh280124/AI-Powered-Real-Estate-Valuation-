...
# ========== HEADER ==========
st.markdown(f'<p class="main-title">{t(lang, "main_title")}</p>',
            unsafe_allow_html=True)
st.markdown(f'<p class="sub-title">{t(lang, "sub_title")}</p>',
            unsafe_allow_html=True)
st.divider()
        ...
        with st.spinner(t(lang, "predicting")):
            try:
                ...
                if is_sale:
                    ...
                    col2.metric(t(lang, "price_est"), f"{predicted:.1f} {t(lang, 'unit_thang')}",
                                delta="XGBoost AI")
                    col3.metric(t(lang, "price_high"), f"{high:.1f} {t(lang, 'unit_thang')}")

                st.info(f"{t(lang, 'price_per_m2')}: **{predicted/area:.1f} {t(lang, 'unit_trieu')}/m²**")

                st.markdown(t(lang, "similar_title"))
                similar = get_similar_properties(df_current, district, city, area, predicted)
                if len(similar) > 0:
                    similar.columns = [
                        t(lang, "col_district"), t(lang, "col_city"),
                        t(lang, "col_area"), t(lang, "col_bed"),
                        t(lang, "col_bath"), t(lang, "col_price"),
                        t(lang, "col_price_m2")
                    ]
...
# ========== TRANG THỊ TRƯỜNG ==========
    ...
    for tab, df, label_key in [
        (tab1, df_sale, "tab_sale"),
        (tab2, df_rental, "tab_rental")
    ]:
        with tab:
            col1, col2 = st.columns(2)
